...
export default function Header({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl bg-slate-100 p-4">
      <h2 className="flex-shrink-0 font-semibold text-zinc-600">Troškovi:</h2>
      <input
      ...
      >
        ...
        <option value="expenseAmount">Vrednost</option>
        <option value="date">Datum</option>
        <option value="description">Opis</option>
        <option value="paidby">Ko je platio</option>
        <option value="group">Grupa</option>
      </select>
      <select
        value={ascDsc}
        onChange={(e) => setAscDsc(e.target.value)}
        className="mx-1 rounded-lg border border-zinc-300 bg-white p-2 transition-colors duration-300 hover:bg-gray-200"
      >
